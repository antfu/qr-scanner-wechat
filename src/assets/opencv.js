...
export const cv = (() => {
  ...
  function writeAsciiToMemory(str, buffer, dontAddNull) {
    for (let i = 0; i < str.length; ++i) HEAP8[buffer++ >> 0] = str.charCodeAt(i)
  ...
  function _emscripten_set_main_loop_timing(mode, value) {
    ...
    if (!Browser.mainLoop.running) Browser.mainLoop.running = true
  ...
  var Browser = {
    ...
    init() {
      ...
      Browser.BlobBuilder =
        typeof MozBlobBuilder != 'undefined' ? MozBlobBuilder : typeof WebKitBlobBuilder != 'undefined' ? WebKitBlobBuilder : !Browser.hasBlobConstructor ? out('warning: no BlobBuilder') : null
    ...
    handledByPreloadPlugin(byteArray, fullname, finish, onerror) {
      Browser.init()
      let handled = false
    ...
    createContext(canvas, useWebGL, setInModule, webGLContextAttributes) {
      ...
      let contextHandle
      if (useWebGL) {
        const contextAttributes = { antialias: false, alpha: false, majorVersion: 1 }
        if (webGLContextAttributes) {
          for (const attribute in webGLContextAttributes) contextAttributes[attribute] = webGLContextAttributes[attribute]
        }
    ...
    requestFullscreen(lockPointer, resizeCanvas) {
      ...
      function fullscreenChange() {
        ...
        if (
          (document.fullscreenElement || document.mozFullScreenElement || document.msFullscreenElement || document.webkitFullscreenElement || document.webkitCurrentFullScreenElement) ===
          canvasContainer
        ) {
    ...
    exitFullscreen() {
      ...
      const CFS = document.exitFullscreen || document.cancelFullScreen || document.mozCancelFullScreen || document.msExitFullscreen || document.webkitCancelFullScreen || function () {}
      CFS.apply(document, [])
      return true
    },
    nextRAF: 0,
    fakeRequestAnimationFrame(func) {
      ...
      if (Browser.nextRAF === 0) {
        ...
        while (now + 2 >= Browser.nextRAF) Browser.nextRAF += 1e3 / 60
      }
    ...
    calculateMouseEvent(event) {
      if (Browser.pointerLock) {
        ...
        if (event.type === 'touchstart' || event.type === 'touchend' || event.type === 'touchmove') {
          ...
          const coords = { x: adjustedX, y: adjustedY }
    ...
    node_ops: {
      ...
      rename(old_node, new_dir, new_name) {
        if (FS.isDir(old_node.mode)) {
          ...
          if (new_node) {
            for (const i in new_node.contents) throw new FS.ErrnoError(55)
          }
        }
    ...
    stream_ops: {
      ...
      write(stream, buffer, offset, length, position, canOwn) {
        ...
        if (node.contents.subarray && buffer.subarray) {
          ...
          for (let i = 0; i < length; i++) node.contents[position + i] = buffer[offset + i]
        }
  ...
  var FS = {
      ...
      for (let i = 0; i < parts.length; i++) {
        ...
        if (!islast || opts.follow) {
          ...
          while (FS.isLink(current.mode)) {
            ...
            const lookup = FS.lookupPath(current_path, { recurse_count: opts.recurse_count + 1 })
            current = lookup.node
    ...
    open: (path, flags, mode) => {
      ...
      const stream = FS.createStream({ node, path: FS.getPath(node), flags, seekable: true, position: 0, stream_ops: node.stream_ops, ungotten: [], error: false })
      if (stream.stream_ops.open) stream.stream_ops.open(stream)
    ...
    createDefaultDevices: () => {
      ...
      const random_device = getRandomDevice()
    ...
    createLazyFile: (parent, name, url, canRead, canWrite) => {
      ...
      stream_ops.read = (stream, buffer, offset, length, position) => {
        ...
        if (contents.slice) {
          ...
          for (var i = 0; i < size; i++) buffer[offset + i] = contents.get(position + i)
        }
        return size
      }
  ...
  function whenDependentTypesAreResolved(myTypes, dependentTypes, getTypeConverters) {
    ...
    function onComplete(typeConverters) {
      ...
      for (let i = 0; i < myTypes.length; ++i) registerType(myTypes[i], myTypeConverters[i])
    }
  ...
  function __embind_finalize_value_array(rawTupleType) {
    const reg = tupleRegistrations[rawTupleType]
    delete tupleRegistrations[rawTupleType]
    const elements = reg.elements
      ...
      return [
        {
          name: reg.name,
          fromWireType(ptr) {
            const rv = new Array(elementsLength)
            for (let i = 0; i < elementsLength; ++i) rv[i] = elements[i].read(ptr)
          ...
          toWireType(destructors, o) {
            ...
            for (let i = 0; i < elementsLength; ++i) elements[i].write(ptr, o[i])
  ...
  function __embind_finalize_value_object(structType) {
    ...
    const fieldTypes = fieldRecords.map((field) => field.getterReturnType).concat(fieldRecords.map((field) => field.setterArgumentType))
      ...
      return [
        {
          ...
          fromWireType(ptr) {
            const rv = {}
            for (const i in fields) rv[i] = fields[i].read(ptr)
  ...
  function embind_init_charCodes() {
    ...
    for (let i = 0; i < 256; ++i) codes[i] = String.fromCharCode(i)
  ...
  function getLiveInheritedInstances() {
    ...
    for (const k in registeredInstances) {
      if (registeredInstances.hasOwnProperty(k)) rv.push(registeredInstances[k])
    }
    return rv
  }
  const deletionQueue = []
  ...
  function RegisteredPointer_fromWireType(ptr) {
    ...
    function makeDefaultHandle() {
      ...
      else return makeClassHandle(this.registeredClass.instancePrototype, { ptrType: this, ptr })
    }
    ...
    if (this.isSmartPointer) return makeClassHandle(toType.registeredClass.instancePrototype, { ptrType: toType, ptr: dp, smartPtrType: this, smartPtr: ptr })
  ...
  function __embind_register_class_class_function(rawClassType, methodName, argCount, rawArgTypesAddr, invokerSignature, rawInvoker, fn) {
    ...
    whenDependentTypesAreResolved([], [rawClassType], (classType) => {
      ...
      whenDependentTypesAreResolved([], rawArgTypes, (argTypes) => {
        ...
        const func = craftInvokerFunction(humanName, invokerArgsArray, null, rawInvoker, fn)
        if (undefined === proto[methodName].overloadTable) {
  ...
  function __embind_register_std_string(rawType, name) {
    ...
    registerType(rawType, {
      ...
      toWireType(destructors, value) {
        ...
        if (stdStringIsUTF8 && valueIsOfTypeString) getLength = () => lengthBytesUTF8(value)
        else getLength = () => value.length
        ...
        if (stdStringIsUTF8 && valueIsOfTypeString) {
          ...
          if (valueIsOfTypeString) {
            ...
            for (var i = 0; i < length; ++i) HEAPU8[ptr + 4 + i] = value[i]
          }
        }
  ...
  function __emval_get_method_caller(argCount, argTypes) {
    ...
    const signatureName = `${retType.name}_$${types
      ...
      .join('_')}$`
    let returnId = emval_registeredMethods[signatureName]
    ...
    for (var i = 0; i < argCount - 1; ++i) {
      if (types[i + 1].deleteObject) functionBody += `    argType${i}.deleteObject(arg${i});\n`
    }
  ...
  function _strftime(s, maxsize, format, tm) {
    ...
    for (var rule in EXPANSION_RULES_2) {
      if (pattern.includes(rule)) pattern = pattern.replace(new RegExp(rule, 'g'), EXPANSION_RULES_2[rule](date))
    }
